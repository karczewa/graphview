  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
      ...
      <button
        ...
        className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-300 dark:disabled:bg-gray-700 disabled:text-gray-400 dark:disabled:text-gray-500 text-white text-sm rounded flex-shrink-0 transition-colors"
      >
        {loading ? '…' : 'Run'}
      </button>
      ...
      <div className="relative flex-shrink-0">
        ...
        {saveDialogOpen && (
          <div className="absolute left-0 top-full mt-1 w-64 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded shadow-xl z-20 p-2 flex gap-2">
            <input
              ref={saveNameRef}
              value={saveName}
              ...
              className="flex-1 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 text-sm px-2 py-1 rounded border border-gray-300 dark:border-gray-700 focus:border-blue-500 focus:outline-none"
            />
            <button
              onClick={handleSave}
      ...
      {savedQueries.length > 0 && (
        <div className="relative flex-shrink-0" ref={savedRef}>
          ...
          {savedOpen && (
            <div className="absolute right-0 top-full mt-1 w-[480px] bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded shadow-xl z-20 max-h-72 overflow-y-auto">
              {savedQueries.map((sq) => (
                <div key={sq.id} className="flex items-center border-b border-gray-300 dark:border-gray-700 last:border-0 hover:bg-gray-200 dark:hover:bg-gray-700 group">
                  <button
                    className="flex-1 text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 font-mono truncate"
                    onClick={() => {
                      setQuery(sq.query);
                      setSavedOpen(false);
                  ...
                  >
                    <span className="text-gray-500 dark:text-gray-400 font-sans mr-2">{sq.name}</span>
                    {sq.query}
                  </button>
                  <button
                    onClick={() => deleteSavedQuery(sq.id)}
                    className="px-3 py-2 text-gray-400 dark:text-gray-600 hover:text-red-400 text-xs opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
          ...
          {historyOpen && (
            <div className="absolute right-0 top-full mt-1 w-[480px] bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded shadow-xl z-20 max-h-72 overflow-y-auto">
              {queryHistory.map((q, i) => (
                <button
                  key={i}
                  className="w-full text-left px-3 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 font-mono truncate border-b border-gray-300 dark:border-gray-700 last:border-0"
                  onClick={() => {
                    setQuery(q);
                    setHistoryOpen(false);
                    setTimeout(autoResize, 0);
      ...
      <select
        ...
        className="px-2 py-1.5 bg-gray-200 hover:bg-gray-300 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm rounded border border-gray-300 dark:border-gray-700 focus:outline-none flex-shrink-0"
