...
export function MindmapModal() {
  ...
  const render = useCallback(async () => {
    ...
    try {
      ...
      // Theme colours for D3-rendered content
      const th = isDark
        ? { link: '#4B5563', nodeStroke: '#111827', text: '#d1d5db' }
        : { link: '#9ca3af', nodeStroke: '#e5e7eb', text: '#1f2937' };
      ...
      const place = (
        ...
        for (const childId of ch) {
          ...
          place(childId, childX, childY, childAngle, childSector * 0.82, d + 1);
        }
      };
  ...
  }, [mindmapNodeId, depth, colorMap, labelShapes, isDark]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white dark:bg-gray-950">
      <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 uppercase tracking-wider">Mind Map</h2>
        <span className="text-xs text-gray-400 dark:text-gray-500">— {mindmapNodeId.split(':').pop()}</span>
        ...
        <button
          ...
          className="px-2.5 py-1 text-xs rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-40 transition-colors"
          title="Export as PNG"
        >
          PNG
        </button>

        {status === 'loading' && <span className="text-xs text-gray-400 dark:text-gray-500">Loading…</span>}
        {status === 'error'   && <span className="text-xs text-red-500 dark:text-red-400">{error}</span>}

        <button
          onClick={() => setMindmapNode(null)}
          className="ml-auto text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 text-xl leading-none transition-colors"
          title="Close (Esc)"
        >
          ✕
        </button>
